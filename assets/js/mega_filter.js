 ...
 * Mega Filter
 */
 var currency = $(".Btn-currency").html();
	...
	*/
	...
	init: function( box, options ) {
		...
		if(options.CallbackFunc) {
			self._ajaxCallback = options.CallbackFunc;
		}
	...
	initResponsive: function(){
		...
		var dirLang = jQuery('html').attr('lang');

		var id = 'mfilter-free-container-' + jQuery('[id^="mfilter-free-container"]').length,
			cnt = jQuery('<div class="mfilter-free-container">')
				.prependTo( jQuery('body') );
		var calculatedcntOuterWidth = cnt.outerWidth(true);

		var	btn = jQuery('<div class="mfilter-free-button">')
				.appendTo( cnt )
				.click(function(){
					if( hidden ) {
						//Please dont' change this new added check, you have to fix css if issue found
						if(dirLang == 'ar'){
							cnt.animate({
								'marginRight' : 0
							}, 500, function(){
						...
						if(dirLang == 'ar'){
							cnt.animate({
								'marginRight' : calculatedcntOuterWidth
							}, 500);
						}else {
							cnt.animate({
	...
	__initLoader: function() {
		this._jqLoader = jQuery('<span class="mfilter-ajax-loader-container" style="cursor: wait; z-index: 100; margin: 0; padding: 0; position: absolute; text-align: center; background-color: rgba(255,255,255,0.7);"></span>')
			...
			.html( '<img src="expandish/view/theme/clearion/stylesheet/mf/images/ajax-loader.gif" alt="" />' )
			.hide();
	},
	...
	getPriceRange: function() {
		...
		var self		= this,
			...
			min			= minInput.val(),
			max			= maxInput.val();		
			$('.ui-slider-range').html('<div class="Range-container"><span class="min">' + $("#mfilter-opts-price-min").val() + currency + '</span>' +'<span class="max">' + $("#mfilter-opts-price-max").val()  + currency + '</span></div>');
	...
	render: function( history ) {
		...
		if( content.length ) {
			...
			self._parseInfo( self.base64_decode( json.html() ) );
